...
{{
  ...
  "potential_issues": ["issue 1", ...]
    ...
    def analyze_code(self, code: str, context: str = "", analysis_type: str = "general") -> str:
        """
        Specialized code analysis method for lineage extraction and STAG comparison

        This method is specifically designed for:
        1. Lineage extraction from workflow code
        2. Column-level lineage tracking
        3. Business purpose analysis
        4. Transformation explanation
        5. Workflow extraction for STAG

        Args:
            code: The code/prompt to analyze
            context: Optional context information
            analysis_type: Type of analysis (lineage_extraction, column_lineage,
                          executive_summary, business_purpose, workflow_extraction, etc.)

        Returns:
            String response from AI (may contain JSON, markdown, or plain text)
        """
        if not self.enabled:
            logger.warning(f"AI analysis disabled for type: {analysis_type}")
            return "{}"  # Return empty JSON for parsers

        try:
            # Build specialized prompts based on analysis type
            if analysis_type == "lineage_extraction":
                system_prompt = """You are an expert in data lineage analysis for ETL pipelines.
Extract table-level lineage from code with high precision.
Return ONLY valid JSON - no markdown, no explanations."""

            elif analysis_type == "column_lineage":
                system_prompt = """You are an expert in column-level lineage tracking.
Extract column transformations and mappings from code.
Return ONLY valid JSON array - no markdown, no explanations."""

            elif analysis_type == "workflow_extraction":
                system_prompt = """You are an expert in extracting workflow information from natural language queries.
Identify source system, target system, and workflow names from user queries.
Return ONLY valid JSON - no markdown, no explanations."""

            elif analysis_type in ["executive_summary", "business_purpose"]:
                system_prompt = """You are a senior data architect explaining data flows to business stakeholders.
Provide clear, concise business-focused explanations.
Return plain text response - 2-3 sentences maximum."""

            elif analysis_type in ["key_insights", "recommendations", "potential_issues"]:
                system_prompt = """You are a data quality and architecture expert.
Provide actionable insights about data pipelines.
Return bullet points, one per line, starting with - """

            elif analysis_type == "transformation_explanation":
                system_prompt = """You are an ETL expert explaining data transformations.
Explain what transformations do in business terms.
Return plain text - 2-3 sentences."""

            elif analysis_type == "transformation_analysis":
                system_prompt = """You are analyzing transformation patterns in data pipelines.
...
Analyze code and provide clear, accurate responses."""
