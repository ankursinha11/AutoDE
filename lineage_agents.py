...
class LogicAgent:
    ...
    def analyze_transformation(
        ...
        if not code_snippet or len(code_snippet.strip()) < 10:
            # Return minimal analysis for empty/tiny code
            return {
                ...
                "dependencies": inputs,
                "outputs": outputs,
                "semantic_meaning": "Transformation performs data processing"
            }

        # Use AI to interpret (all methods now use AI internally)
        analysis = {
            "transformation_type": transform_type,
            ...
            "complexity_score": self._assess_complexity(code_snippet),  # Alias for compatibility
            "dependencies": inputs,
            "outputs": outputs,
            "semantic_meaning": self._extract_semantic_meaning(code_snippet)
        }

        logger.debug(f"Analysis complete: {analysis['business_purpose'][:50]}...")
        return analysis

    def _infer_business_purpose(self, code: str) -> str:
        """Infer business purpose from code using AI"""
        ...
        try:
            # Use AI to infer business purpose
            prompt = f"""Analyze this code snippet and infer its business purpose in ONE concise sentence (10-15 words max).
...
Return ONLY the business purpose sentence, nothing else."""
            ...
            return purpose.strip() if purpose else "Data transformation"

        except Exception as e:
            logger.warning(f"AI business purpose inference failed: {e}")
            return "Data transformation"

    def _detect_quality_rules(self, code: str) -> List[str]:
        """Detect data quality rules using AI"""
        if not self.ai_analyzer or not self.ai_analyzer.enabled:
            # Fallback to keyword matching if AI not available
            rules = []
            code_lower = code.lower()
        ...
        try:
            # Use AI to detect quality rules
            prompt = f"""Analyze this code and identify data quality rules being applied.
...
List each data quality rule on a separate line starting with "- ". Examples:
- Null value validation on customer_id field
- Deduplication based on order number
- Date format standardization
- Email address validation

Return bullet points only, no explanations."""
            ...
            logger.warning(f"AI quality rule detection failed: {e}")
            return []

    def _assess_complexity(self, code: str) -> str:
        """Assess transformation complexity using AI"""
        if not self.ai_analyzer or not self.ai_analyzer.enabled:
        ...
        try:
            ...
            prompt = f"""Assess the complexity of this code transformation. Consider:
- Number of operations and transformations
- Data flow complexity
- Logic nesting and conditionals
...
Return ONLY one word: LOW, MEDIUM, or HIGH"""
            ...
            complexity = self.ai_analyzer.analyze_code(
                code=prompt,
                context="",
            ...
            if "HIGH" in complexity_upper:
                return "HIGH"
            elif "MEDIUM" in complexity_upper:
                return "MEDIUM"
            ...
            logger.warning(f"AI complexity assessment failed: {e}")
            return "MEDIUM"

    def _extract_semantic_meaning(self, code: str) -> str:
        """Extract semantic meaning of transformation using AI"""
        if not self.ai_analyzer or not self.ai_analyzer.enabled:
            # Fallback to generic description
            return "Transformation performs data processing on input streams"

        try:
            # Use AI to generate natural language description
            prompt = f"""Explain what this code transformation does in simple, business-friendly language (2-3 sentences).
...
Write in present tense, be specific about the operations."""

            meaning = self.ai_analyzer.analyze_code(
                code=prompt,
                context="",
                analysis_type="transformation_explanation"
            )

            return meaning.strip() if meaning else "Transformation performs data processing on input streams"

        except Exception as e:
            logger.warning(f"AI semantic meaning extraction failed: {e}")
            return "Transformation performs data processing on input streams"
